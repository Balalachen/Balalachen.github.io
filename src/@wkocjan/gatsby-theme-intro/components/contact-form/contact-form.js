...
const ContactForm = ({ formspreeEndpoint, budget }) => {
  ...
  return (
    <>
      <h5
        className="font-header font-semibold text-front dark:text-front-dark text-sm uppercase mt-12 mb-3 transition-colors duration-500"
        id="contact"
      >
        Contact
      </h5>
      ...
      <form action={formspreeEndpoint} className="flex flex-wrap" method="post">
        ...
        <div className="w-full text-right py-px lg:p-px">
          ...
          <div className="bg-back-light dark:bg-back-light-dark">
            <input
              className="font-header inline-block font-semibold px-6 py-2 leading-tight cursor-pointer outline-none bg-front text-back-light dark:bg-front-dark dark:text-back-light-dark uppercase mr-4 my-4 transition-opacity duration-150 hover:opacity-75"
              type="submit"
              value="Send"
            />
          </div>
