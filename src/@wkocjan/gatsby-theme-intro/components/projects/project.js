...
const Project = props => {
  ...
  return (
    <div
      className="overflow-hidden transition-all duration-150 ease-linear mb-3"
      style={{
        ...
        {awards &&(
          <ul className={"pr-2 flex flex-col w-1/2" + ((image == null) ? "" : "lg:w-full") }>
            {awards.map(awardLine =>(
              <li className="inline-block flex flex-row font-medium mt-1">
                {awardLine.includes("[a] ") ? 
                  <GiLaurelsTrophy className="inline-block w-4 h-4" /> :
                  <GiLaurels className="inline-block w-4 h-4" />}
                <span className="flex-1 pl-2 text-xs">{awardLine.replace("[a] ", "")}</span>
              </li>
            ))}
          </ul>
        )}
        {exhibitions &&(
          <ul className={"pr-2 flex flex-col lg:mt-2 w-1/2" + ((image == null) ? "" : "lg:w-full") }>
            {exhibitions.map(exhib =>(
              <li className="flex flex-row font-medium mt-1 ml-0">
